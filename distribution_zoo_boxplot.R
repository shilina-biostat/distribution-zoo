...
# Trim rare extreme values so the shape stays visible.
# Without this, rt(df = 1.5) especially can stretch the x-axis
# until the actual shape becomes invisible.
...
dark_theme <- theme_minimal(base_size = 13) +
  ...
  theme(
    ...
    # Intentional here: a density-histogram y-axis carries no
    # meaningful units, so it stays hidden for Part 1 only.
...
# Save
ggsave(
  "outputs/distribution_zoo_dark.png",
...
make_boxplot <- function(data, label, y_limits) {
  ...
  ggplot(data.frame(x = "Group", y = data), aes(x = x, y = y)) +
    ...
    theme(
      ...
      axis.text.y = element_text(color = "#aaaaaa")   # restore the y-scale for Part 2
    )
}
...
ggsave(
  "outputs/boxplot_vs_raincloud_dark.png",
...
write.csv(
  guidance,
  "outputs/distribution_guidance.csv",
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
